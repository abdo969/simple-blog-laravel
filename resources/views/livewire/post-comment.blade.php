<div class="mt-10 comments-box border-t border-gray-100 pt-10">
    ...
    <div class="user-comments px-3 py-2 mt-5">
        @forelse($comments as $comment)
            <div class="comment [&:not(:last-child)]:border-b border-gray-100 pt-7">
                <div class="user-meta flex mb-4 text-sm items-center">
                    <x-posts.author :author="$comment->user" size="md"/>
                    <span class="text-gray-500">. {{ $comment->created_at->diffForHumans() }}</span>
                    <button wire:click="deleteComment({{ $comment->id }})" class="ml-4 text-red-500 text-xs">Delete</button>
                </div>
            </div>
            <div class="text-justify text-gray-700 mt-2 text-sm">
                {{ $comment->comment }}
            </div>
        @empty
