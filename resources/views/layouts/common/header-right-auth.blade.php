...
<div class="ms-3 relative flex space-x-4">
    ...
    @endcan
    <x-dropdown align="right" width="18">
        <x-slot name="trigger">
            <button
                class="flex text-sm border-2 border-transparent rounded-full transition">
                @if (session('locale'))
                    <x-dynamic-component :component="'flag-country-'.config('supportedlocale.'.session('locale').'.icon')" class="h-6 w-6 mt-1"/>
                @endif
            </button>
        </x-slot>
        <x-slot name="content">
            <!-- Language Selection -->
            @foreach (config('supportedlocale') as $lang => $data)
                <x-dropdown-link wire:navigate href="{{ route('locale', $lang) }}">
